...
function onCreate() {
    ...
    convertToOld('.json'); // this is where you change to JUST the json name syn. Add .json if it doesn't do anything.

    // Converting current song and difficulty
    convertToOld('data/'+game.songName+'/'+game.songName+Difficulty.getFilePath()); //keep this syn
    ...
    convertToOld('data/'+game.songName+'/events'); // you keep this syn
    return;
}
